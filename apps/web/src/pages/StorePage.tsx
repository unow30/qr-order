      ...
      <div className="flex flex-col min-h-screen bg-zinc-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1 min-w-0">
                ...
                <h1 className="text-base sm:text-lg font-extrabold text-zinc-900 tracking-[-0.3px]">
                  {selectedStore.name}
                </h1>
              </div>
              <p className="hidden sm:block text-[11px] text-zinc-500 leading-relaxed text-right">
                테이블의 QR 코드를 스캔하거나 입장 링크를 클릭하세요
              </p>
            </div>
            <p className="sm:hidden mt-1 text-[11px] text-zinc-500 leading-relaxed">
        ...
        <div className="flex-1 px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="max-w-7xl mx-auto">
            ...
            {error && (
              <div className="px-4 py-2.5 bg-red-50 text-[11px] text-red-700 rounded-[10px] mb-4">
                {error}
              </div>
            )}

            {!tablesLoading && !error && tables.length === 0 && (
              <p className="text-[11px] text-zinc-500 text-center py-16">
                등록된 테이블이 없습니다
              </p>
            )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h1 className="text-base sm:text-xl font-extrabold text-zinc-900 tracking-[-0.3px]">
                매장 목록
              </h1>
            </div>
            <p className="hidden sm:block text-[11px] text-zinc-500 leading-relaxed text-right">
      ...
      <div className="flex-1 px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="max-w-7xl mx-auto">
          ...
          {!loading && !error && stores.length === 0 && (
            <p className="text-[11px] text-zinc-500 text-center py-16">등록된 매장이 없습니다</p>
          )}
              ...
              <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {stores.map((store) => (
                  <div
                    ...
                    className={`bg-white rounded-xl p-4 sm:p-5 border border-zinc-200 transition-colors ${
                      store.isActive
                        ? 'cursor-pointer active:bg-zinc-50 hover:border-zinc-300 hover:shadow-sm'
                        : 'opacity-45'
                    }`}
                  >
                    <div className="flex items-center gap-2 mb-1">
