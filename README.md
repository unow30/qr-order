...
| 앱 | 설명                  | 주소                                          | README |
|----|---------------------|---------------------------------------------|--------|
| `apps/server` | NestJS REST API 백엔드 | https://server.qr-order-demo.it.kr/         | [바로가기](./apps/server/README.md) |
| `apps/server` | swagger             | https://server.qr-order-demo.it.kr/api/docs | [바로가기](./apps/server/README.md) |
| `apps/web` | 고객용 모바일 웹앱          | https://www.qr-order-demo.it.kr/            | [바로가기](./apps/web/README.md) |
...
```bash
pnpm dev:server      # docker 서버만 실행
pnpm dev:web         # docker 고객 앱만 실행
pnpm dev:admin       # docker 어드민만 실행
pnpm build           # docker server,web,admin 전체 빌드
